...
// AssemblyAI API has been implemented in src/utils/assemblyAIApi.ts
// Import and use assemblyAIAPI from that file instead of this mock
...
export const mockAssemblyAI = {
  startTranscription(onTranscript: (text: string) => void, onComplete: () => void) {
    ...
    */
    ...
    const mockPhrases = [
      "So basically, this concept is about the documents that guide the behaviour and conduct of people in specific professions",
      "The main idea here is that everyone should follow these rules to stay compliant",
      "What I understand from this is that the rules are there to help people do their jobs well",
      "There are different codes of conduct for different professions like law has the Bar Association in the US, and medicine has the AMA while computing professionals have the ACM"
    ];
...
// ElevenLabs API has been implemented in src/utils/elevenLabsApi.ts
// Import and use elevenLabsAPI from that file instead of this mock